...
//其中UR是产生一个0 -（limit-1）的随机数
  ...
  for (i = 0; i < cnt - 2; i++) {
//在i到cnt-1内随机产生一个索引下标j。将数组元素nl[i] 与 nl[j] 位置互换
    u32 j = i + UR(cnt - i);
    void *s = ptrs[i];
    ptrs[i] = ptrs[j];
    ptrs[j] = s;
...
//打开fn对应的文件，若没有则创建。
  ...
  ck_free(fn);
//设置queue中这一项的passed_det=1代表已经fuzz过了
  q->passed_det = 1;
...
//如果state设置了
  if (state) {
//尝试创建并打开out_dir/queue/.state/redundant_edges/q->fname
    fd = open(fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
    if (fd < 0) PFATAL("Unable to create '%s'", fn);
    close(fd);
  //否则删除fn
...
//上面设置的
  ...
  //如果current不为0(命中了相应路径)，且*current&*virgin不为0代表我们命中的某个bit之前是没有碰到过的新的）
...
static void simplify_trace(u32* mem) {
  ...
  while (i--) {
    ...
    if (unlikely(*mem)) {
      ...
      u8* mem8 = (u8*)mem;
      //代表当命中了（mem8[i]=1）时，对应的是mem8[i]被设置为128
    //没命中被设置为mem8[i]被设置为1
      mem8[0] = simplify_lookup[mem8[0]];
      mem8[1] = simplify_lookup[mem8[1]];
      mem8[2] = simplify_lookup[mem8[2]];
      mem8[3] = simplify_lookup[mem8[3]];
    //当(*mem)为0时
    //设置这一组为0x0101010101010101，代表都没有命中，每个字节被置1
    } else *mem = 0x01010101;
//mem++，后移到下一组
    mem++;
  }
...
//本函数用来初始化u16 count_class_lookup16[65536]这个数组
//初始化 count_class_lookup16 实际是因为 AFL 中对于一条分支径的表示是由一个二元组来表示的
  u32 b1, b2;
//将整个 count_class_lookup16 分成256段，每一段256份儿
  for (b1 = 0; b1 < 256; b1++) 
    for (b2 = 0; b2 < 256; b2++)
      count_class_lookup16[(b1 << 8) + b2] = 
        (count_class_lookup8[b1] << 8) |
        count_class_lookup8[b2];
//count_class_lookup8中对于执行次数进行了规整，
//比如执行了4-7次的其计数为8，比如32次到127次都会认为是64次
// 变量trace_bits来记录分支执行次数，而count_class_lookup8实际就是对于trace_bits的规整
}
//
...
//*(src++)每次取trace_bits[]数组中一个元素的值
  while (i < MAP_SIZE) {
//i&7相当于i%8,相当于计算我们要放到dst的一个byte中的哪个bit
...
//当我们碰到一个新路径时，判断此路径是否是更有利的
//即是否是能遍历到bitmap中的bit的最小的路径集合
  u32 i;
  //首先在开始时定义了一个偏好因子fav_factor = q->exec_us * q->len
...
//接下来相当于重新更新对应的top_rated[i]中维护的相关数据
       top_rated[i] = q;
       q->tc_ref++;
//如果q->trace_mini为0
       if (!q->trace_mini) {
         ...
         //对原trace_bits进行压缩，放到只有原来1/8大小的q->trace_mini中
...
//从queue队头开始扫描，设置队列中每一项的 q->favored = 0
  ...
  for (i = 0; i < MAP_SIZE; i++)
  //如果对于当前queue entrytop_rated[i]被设置了，并且对应的temp_v[i >> 3]的第(1 << (i & 7))bit被置位。（即对应的path是否置位）
...
//当申请共享内存成功后，通过设置析构函数atexit()，调用 shmctl 删除这一段共享内存
  ...
  /* If somebody is asking us to fuzz instrumented binaries in dumb mode,
     ...
     later on, perhaps? */
//如果没有设置dumb_mode（哑模式）
//那么设置SHM_ENV_VAR为shm_str（此时是65539）
...
static void setup_post(void) {
  ...
  u32 tlen = 6;
  //没有AFL_POST_LIBRARY环境变量就直接return
  if (!fn) return;
...
//通过lstat(fn, &st) || access(fn, R_OK)检测目录是否可访问
//从lstat(fn, &st)获取的文件信息保存在struct stat st中
    if (lstat(fn, &st) || access(fn, R_OK))
      PFATAL("Unable to access '%s'", fn);

    /* This also takes care of . and .. */
//检查我们扫描到的文件是否是有效的常规文件
//通过S_ISREG查看st.st_mode确定文件的性质
//st.st_size 确定大小；strstr查找是否是README.txt文件
...
//打开对应dir
  ...
  if (!d) {
//打开失败且 errno == ENOTDIR ( 不是一个目录文件)
    if (errno == ENOTDIR) {
      load_extras_file(dir, &min_len, &max_len, dict_level);
      //对这个file进行检查和排序（by size）
      goto check_and_sort;
    }
...
//MAX_AUTO_EXTRAS代表最大的auto_extras token数量
//USE_AUTO_EXTRAS代表用户指定的auto_extras token数量，默认50
  if (!MAX_AUTO_EXTRAS || !USE_AUTO_EXTRAS) return;

  /* Skip runs of identical bytes. */
//接下来从mem[1]开始扫描，跳过与mem[0]相同的byte。
//最后索引i停止在第一个与mem[0]不相同的位置
  for (i = 1; i < len; i++)
    if (mem[0] ^ mem[i]) break;
//i==len那么所有mem中的byte都相同
  if (i == len) return;
...
//用realloc调整a_extras_cnt的空间为a_extras_cnt+1
...
static void save_auto(void) {
  ...
  for (i = 0; i < MIN(USE_AUTO_EXTRAS, a_extras_cnt); i++) {
    ...
    if (fd < 0) PFATAL("Unable to create '%s'", fn);
    //扫描a_extras数组，将对应的data写入fn中，fn是前面补全的路径
...
static void load_auto(void) {
  ...
  for (i = 0; i < USE_AUTO_EXTRAS; i++) {
    ...
    s32 fd, len;
    //以0600权限打开补全路径对应的文件，返回fd。如果open失败则abort掉
    ...
    /* We read one byte more to cheaply detect tokens that are too
       long (and skip them). */
    //多读取一个字节来判断是否读取的token过长
    len = read(fd, tmp, MAX_AUTO_EXTRA + 1);
    //没读到
    if (len < 0) PFATAL("Unable to read from '%s'", fn);
    //加进去，最后一次可能读不到那么多所以有小于等于
    if (len >= MIN_AUTO_EXTRA && len <= MAX_AUTO_EXTRA)
    //将我们此时的语料加入a_extras[]数组中
    //此时tmp(mem)对应的是读取的auto extra文件，len对应长度
...
//首先进行 setsid()创建新的会话，使子进程完全独立，脱离控制
    setsid();
  //接下来重新设置fd，关闭子进程中的stdout和stderr,然后将其重定向到/dev/null中
  //相当于关闭了子进程的全部输出
    dup2(dev_null_fd, 1);
    dup2(dev_null_fd, 2);
//如果设置了outfile，那么用dup2再关闭掉子进程的stdin，重定向到/dev/null
    if (out_file) {

      dup2(dev_null_fd, 0);
    //否则关闭stdin，重定向到out-fd，最后关闭out_fd
    } else {
  ...
  //将FORKSRV_FD重定向到ctl_pipe[0]
    if (dup2(ctl_pipe[0], FORKSRV_FD) < 0) PFATAL("dup2() failed");
    //将FORKSRV_FD + 1重定向到st_pipe[1]
...
//在父进程中关闭ctl_pipe[0],与st_pipe[1],控制管道读与状态管道写，因为父进程不需要
  close(ctl_pipe[0]);
  close(st_pipe[1]);
//对父进程进行如下赋值
...
//等待forkserver启动
  ...
  //如果设置了child_timed_out,则同值用户调整-t参数
  if (child_timed_out)
    FATAL("Timeout while initializing fork server (adjusting -t may help)");
//否则等forksrv是否返回status
  if (waitpid(forksrv_pid, &status, 0) <= 0)
    PFATAL("waitpid() failed");
//判断forksrv是否返回的异常退出的信号status
  if (WIFSIGNALED(status)) {
    //是否设置 mem_limit && mem_limit < 500 && uses_asan （mem_limit过小）
    ...
    //最后直接通过WTERMSIG(status) 获取信号告知用户single值
...
//执行一段与init_forkserver中非常相似的代码。（没有forkserver时，在子进程中执行targetfile）
  if (dumb_mode == 1 || no_forkserver) {
      //fork一个子进程，在子进程中执行target_path
    child_pid = fork();
    //若失败
  ...
  //否则代表此时已经有一个fork server up and running了
  } else {
...
//如果没有设置from_queue，或者设置了resuming_fuzz（恢复会话或尝试校准已添加的发现时）
...
//判断此时的forkserver是否已经启动
  if (dumb_mode != 1 && !no_forkserver && !forksrv_pid)
  //若未启动则调用init_forkserver(argv)
...
//进入一个大的for循环
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    u32 cksum;
    //如果不是first_run,且经过了一个周期，那就输出fuzz信息
    if (!first_run && !(stage_cur % stats_update_freq)) show_stats();
//将读取的内容写入我们的outfile中
    write_to_testcase(use_mem, q->len);
//调用这个运行文件，返回结果存储在fault中
    ...
    //stage_cur==0（第一次运行）
    //count_bytes查找trace_bit中是否有被设置为1的bit
    if (!dumb_mode && !stage_cur && !count_bytes(trace_bits)) {
      fault = FAULT_NOINST;
      goto abort_calibration;
    }
//根据当前的trace_bits计算cksum
    cksum = hash32(trace_bits, MAP_SIZE, HASH_CONST);
//如果当前的q->exec_cksum != cksum
    if (q->exec_cksum != cksum) {
      //查看是否有新的tuple产生还是仅有hit-count改变
      u8 hnb = has_new_bits(virgin_bits);
      //如果返回的hnb > new_bits，一开始是0，更新new_bits=hnb
...
//i为索引，扫描var_bytes、first_trace、trace_bits
        ...
        for (i = 0; i < MAP_SIZE; i++) {
          ...
          if (!var_bytes[i] && first_trace[i] != trace_bits[i]) {
            //设置对应的var_bytes[i] = 1 ，令stage_max = CAL_CYCLES_LONG
            var_bytes[i] = 1;
...
//计算当前这一queue entry的q->exec_us
  q->exec_us     = (stop_us - start_us) / stage_max;
//计算执行完毕后trace_bits中的路径信息
...
static void check_map_coverage(void) {
  ...
  u32 i;
//利用count_bytes(trace_bits)统计发现的路径数量，小于100则return
  if (count_bytes(trace_bits) < 100) return;
  //扫描trace后半部分，如果有不为0的就返回
...
//打印提示信息后进入一个大switch。switch (res)
    switch (res) {
      ...
      case FAULT_NONE:
//如果q此时是queue头，那么调用check_map_coverage()检测map覆盖率
...
//尝试以只读+0600权限打开new_path，同时使用O_CREAT | O_EXCL测试文件是否存在
  dfd = open(new_path, O_WRONLY | O_CREAT | O_EXCL, 0600);
  if (dfd < 0) PFATAL("Unable to create '%s'", new_path);
//分配64k的空间为缓冲区
  tmp = ck_alloc(64 * 1024);
//通过缓冲区中转，将old_path中的内容写入new_path
...
//   指向input case queue 队头
  ...
  while (q) {
//首先通过 *rsl = strrchr(q->fname, '/') 扫描queue中第一个元素的fname
//并从取出最后一个'/'以及其后的内容
    u8  *nfn, *rsl = strrchr(q->fname, '/');
    u32 orig_id;
  //如果读取成功了，则rsl++跳过'/'
    if (!rsl) rsl = q->fname; else rsl++;
  //如果读取失败，说明没有'/'，直接令rsl=q->fname
...
//如果rsl以CASE_PREFIX开头，尝试将CASE_PREFIX后的数字以 "%06u" 格式化后存入orig_id
    if (!strncmp(rsl, CASE_PREFIX, 3) &&
        sscanf(rsl + 3, "%06u", &orig_id) == 1 && orig_id == id) {
          //若此时的orig_id与id相等，设置resuming_fuzz = 1
      u8* src_str;
      u32 src_id;
...
//接下来使用 strchr(rsl + 3, ':') 跳过 CASE_PREFIX 查找下一个 ":" 的位置，存储在src_str中
      src_str = strchr(rsl + 3, ':');
//如果 src_str 存在，用sscanf将其后的数字以 "%06u" 格式化后存入src_id
      if (src_str && sscanf(src_str + 1, "%06u", &src_id) == 1) {
        //接下来让指针s从队头开始扫描，每扫描过一个元素，src_id--
        struct queue_entry* s = queue;
        while (src_id-- && s) s = s->next;
        if (s) q->depth = s->depth + 1;
        //判断队列深度是否超过最大深度，如果超过最大深度，则指定为最大深度
...
//调用 link_or_copy(q->fname, nfn) 创建硬链接（q->fname到nfn）
//并将input/xxx.out文件中的内容写入output/queue/id:000000,org:xxx.out
...
//如果设置了 q->passed_det=1
//那么调用mark_as_det_done(q)标记queue这一项已经fuzz过了，并保持q->passed_det=1
...
//遍历结束后检测是否设置in_place_resume，
//若设置了调用 nuke_resume_dir() 删除 output/_resume/*临时目录。这个目录主要用于本地临时恢复。
...
//本函数用于检测我们在run_target中运行的文件返回的结果是否是“有趣的”
//进而确定是否要在未来的分析时保存或者插入队列中。若需要返回1，否则返回0
  u8  *fn = "";
  u8  hnb;
  s32 fd;
  u8  keeping = 0, res;
//如果fault等于crash_mode
  if (fault == crash_mode) {
    ...
    if (!(hnb = has_new_bits(virgin_bits))) {
      //没有newbits，若设置了crash_mode，则total_crashes计数加一
...
//利用hash32从新计算trace_bits的哈希值
    queue_top->exec_cksum = hash32(trace_bits, MAP_SIZE, HASH_CONST);

    /* Try to calibrate inline; this also calls update_bitmap_score() when
       successful. */
//调用calibrate_case进行用例校准，评估当前队列
...
//若没有新的超时路径，直接returnkeeping
      ...
      if (exec_tmout < hang_tmout) {
        ...
        //然后再次调用run_target运行一次，返回new_fault
        new_fault = run_target(argv, hang_tmout);

        /* A corner case that one user reported bumping into: increasing the
           timeout actually uncovers a crash. Make sure we don't discard it if
           so. */
        //如果未设置stop_soon，且。。则跳到
      ...
      unique_hangs++;
      //通过get_cur_time()获取last_hang_time
...
//设置remove_len,即len_p2/16，与4中最大的那个，作为步长
  remove_len = MAX(len_p2 / TRIM_START_STEPS, TRIM_MIN_BYTES);

  /* Continue until the number of steps gets too high or the stepover
     gets too small. */
//通过一个while循环调整步长，直到变得太长或太短
//每轮remove_len/2
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, TRIM_MIN_BYTES)) {

    u32 remove_pos = remove_len;
//通过sprintf格式化remove_len到tmp中
...
//写出修改后的测试用例，运行程序，处理result与错误等
  ...
  //如果设置了post_handler
  if (post_handler) {
    //调用post_handler(out_buf, &len)，返回out_buf
    out_buf = post_handler(out_buf, &len);
    if (!out_buf || !len) return 0;

  }
  //调用write_to_testcase写出out_buf
  write_to_testcase(out_buf, len);
  //run_target运行要fuzz的程序，返回fault
  fault = run_target(argv, exec_tmout);
  ...
  //save_if_interesting检测我们在run_target中运行的文件返回的结果是否是“有趣的”
  //返回值加上queued_discovered成为新的queued_discovered
  queued_discovered += save_if_interesting(argv, out_buf, len, fault);
  //根据运行的轮数来决定是否输出fuzz统计信息
...
static u8 fuzz_one(char** argv) {
  ...
  if (pending_favored) {
    ...
    //并且打一个100以内的随机数，如果小于SKIP_TO_NEW_PROB(99%)直接return1
        UR(100) < SKIP_TO_NEW_PROB) return 1;
    //如果不是dumb_mode，且当前的不是favored，并且queued_paths>10
  } else if (!dumb_mode && !queue_cur->favored && queued_paths > 10) {

    /* Otherwise, still possibly skip non-favored cases, albeit less often.
       The odds of skipping stuff are higher for already-fuzzed inputs and
       lower for never-fuzzed entries. */
    //如果这个大于1，且当前的queue_cur还没有被fuzz过
    if (queue_cycle > 1 && !queue_cur->was_fuzzed) {
      //打出一个100以内的随机数，如果小于这个就直接return 1 (75%)
      if (UR(100) < SKIP_NFAV_NEW_PROB) return 1;

    } else {
      //否则打一个100以内的随机数，如果小于这个直接retuen 1(95%)
  ...
  len = queue_cur->len;
  //将当前的test case映射进入内存，ucore时候好像用过这个函数
...
//修剪阶段
  ...
  //如果设置了skip_deterministic或者queue_cur->was_fuzzed（被fuzz过了）
  //或者queue_cur->passed_det=1
  ...
  /* Skip deterministic fuzzing if exec path checksum puts this out of scope
    ...
    //如果当前的queue_cur->exec_cksum % master_max不等于master_id - 1，且那个
...
//首先分配len大小的空间eff_map
  ...
  //将eff_map[0]初始化为1
  eff_map[0] = 1;
//将eff_map[(len - 1)>>3]初始化为1（即第一项和最后一项）
  if (EFF_APOS(len - 1) != 0) {
  ...
  /* Walking byte. */
//进入"bitflip 8/8"阶段
//本阶段有一个很重要的思想：我们通过对于 out_buf所有bit进行异或翻转，如果产生了不一样的路径，就在eff_map中标记为1，否则为0
//因为如果对所有bit都做翻转还无法带来相关的路径变化，afl认为在后续的一些开销更大的阶段，参考eff_map，可以对这些无效的byte进行跳过。减小开销
...
//这个阶段中不是通过FILP宏来做翻转，而是直接与0xff做异或
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (!eff_map[EFF_APOS(stage_cur)]) {
      ...
      //如果不是dumb_mode且len大于最小的EFF_MIN_LEN
      if (!dumb_mode && len >= EFF_MIN_LEN)
      //计算hash32
        cksum = hash32(trace_bits, MAP_SIZE, HASH_CONST);
       //否则，如果是dumb mode或者len过小
      else
...
//唯一不同的是，在异或变异之前先检查了对应的eff_map的对应两个字节是否为0
    if (!eff_map[EFF_APOS(i)] && !eff_map[EFF_APOS(i + 1)]) {
      //如果是0，stage_max计数减1.然后continue跳过
      stage_max--;
      continue;
    }
    //否则进行异或翻转后运行
...
//本阶段主要做加减变异
  ...
  stage_short = "arith8";
  stage_cur   = 0;
  stage_max   = 2 * len * ARITH_MAX;  //ARITH_MAX=35
  ...
  for (i = 0; i < len; i++) {
    ...
    if (!eff_map[EFF_APOS(i)]) {
      //则stage_max减去2倍的ARITH_MAX，然后continue跳过此次变异
...
//本阶段主要做替换变异
  ...
  for (i = 0; i < len; i++) {
    ...
    for (j = 0; j < sizeof(interesting_8); j++) {
      ...
      //然后通过out_buf[i] = interesting_8[j]进行一个字节的替换
...
//以两个字节为单位进行替换变异，并且去除异或、加减、与单字节变异阶段的冗余，同时考虑大小端序
...
skip_extras:
/*
如果我们在不跳至havoc_stage或abandon_entry的情况下来到这里，
说明我们已经正确的完成了确定的fuzz（deterministic steps）步骤，
我们可以对其进行标记如 .state/ 目录
*/
  /* If we made this to here without jumping to havoc_stage or abandon_entry,
     we're properly done with deterministic steps and can mark it as such
     in the .state/ directory. */
//如果没有设置queue_cur->passed_det
//调用mark_as_det_done(queue_cur)进行标记
...
//设置stage_max
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
//在每一轮stage中首先产生随机数use_stacking
    u32 use_stacking = 1 << (1 + UR(HAVOC_STACK_POW2));

    stage_cur_val = use_stacking;
//根据产生的use_stacking做相应次数的变换。相当于每一轮stage中具体的变换由多次小变化叠加产生
    for (i = 0; i < use_stacking; i++) {
//每次变换具体的内容也由一个随机数决定
      switch (UR(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0))) {
//以下操作是之前见过的那些个变异，在这里又随机来了一遍
        case 0:
...
//至此，叠加变化结束，对进行这些随机大变换后的进行fuzz
    ...
    /* If we're finding new stuff, let's run for a bit longer, limits
       permitting. */
//如果fuzz后的queued_paths与havoc_queued不一样了，说明发现了新路径
...
//当没有define IGNORE_FINDS时
#ifndef IGNORE_FINDS
//如果我们经过了一整轮什么都没有发现，那么afl会进入retry_splicing:
  /************
   * SPLICING *
   ************/
...
retry_splicing:
//这里进一步的对于输入样本进行变换，通过拼接另一个输入样本来完成此变换，
//最后又跳回havoc_stage上一阶段进行大范围的随机变换
...
//本函数用于抓取其他fuzzer的case，读取其他fuzz文件夹下的文件调用，
//记录下来我们最后一个打开的文件的qd_ent->d_name，
//最后将其写到out_dir/.synced/sd_ent->d_name中
...
//然后通过readdir(sd)返回一个指向struct dirent的指针sd_ent
//接下来通过一个while循环遍历sync_dir目录下的所有文件。（由其他的fuzzer创建的
...
//打开out_dir/.synced/sd_ent->d_name，返回到id_fd
    id_fd = open(qd_synced_path, O_RDWR | O_CREAT, 0600);

    if (id_fd < 0) PFATAL("Unable to create '%s'", qd_synced_path);
//接着从打开的id_fd读取一个sizeof(u32)到min_accept
    if (read(id_fd, &min_accept, sizeof(u32)) > 0) 
    //然后lseek从新调整文件内指针到开头
      lseek(id_fd, 0, SEEK_SET);
//设置当前的next_min_accept为我们刚刚读取的min_accept
    next_min_accept = min_accept;

    /* Show stats */    
//sync_cnt计数加一，由"sync %u"格式化到stage_tmp中
...
//如果没有，执行此个测试用例
    //利用readdir(qd)进一步取出目录中的文件
    while ((qd_ent = readdir(qd))) {

      u8* path;
      s32 fd;
      struct stat st;
      //若文件以'.'开头，或者syncing_case < min_accept
      //或者；或者我们使用sscanf(qd_ent->d_name, CASE_PREFIX "%06u", &syncing_case)失败返回-1
      //就跳过
      if (qd_ent->d_name[0] == '.' ||
      ...
      //如果syncing_case >= next_min_accept
      if (syncing_case >= next_min_accept)
      //设置next_min_accept为syncing_case + 1
        next_min_accept = syncing_case + 1;

      path = alloc_printf("%s/%s", qd_path, qd_ent->d_name);

      /* Allow this to fail in case the other fuzzer is resuming or so... */
      //打开qd_path/qd_ent->d_name返回为fd
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        //调用write_to_testcase(mem, st.st_size)将其写到outfile中
        write_to_testcase(mem, st.st_size);
        //接着run_target运行对应文件，返回fault
        fault = run_target(argv, exec_tmout);
        //如果设置了stop_soon，直接返回
        if (stop_soon) return;
        //将syncing_party置为sd_ent->d_name
...
static void handle_timeout(int sig) {
  ...
  //child_pid == -1 且 forksrv_pid > 0时杀死forkserver进程
  //-1是定义的初始值，不是返回值
...
//设置输出的文件夹与fd
  ...
  if (mkdir(out_dir, 0700)) {
    ...
    //调用open以读权限打开，返回fd，赋值给out_dir_fd
    out_dir_fd = open(out_dir, O_RDONLY);
...
//将out_dir与/queue拼接为tmp。（output/queue）
...
//以下重复上面工作各种创建文件夹
  ...
  ck_free(tmp);
//通过fdopen将fd转为文件指针plot_file
  plot_file = fdopen(fd, "w");
  if (!plot_file) PFATAL("fdopen() failed");
  //通过fprintf将一些信息写入文件 忽略错误
...
static void fix_up_sync(void) {
  ...
  //拼接out_ir和sync_id为新的out_dir
  x = alloc_printf("%s/%s", out_dir, sync_id);

  sync_dir = out_dir;
  out_dir  = x;
  //如果force_deterministic没有设置
  if (!force_deterministic) {
    //这俩设为1
...
//一个大while扫描argv，查找是否有@@字串
  while (argv[i]) {
    //如果找到了@@字串，位置存在aa_loc中
...
//若没设置out_file：拼接产生out_file
      if (!out_file)
        out_file = alloc_printf("%s/.cur_input", out_dir);

      /* Be sure that we're always using fully-qualified paths. */
//若果 out_file[0] == '/' 那么直接令：aa_subst = out_file。否则拼接产生out_file
      if (out_file[0] == '/') aa_subst = out_file;
      else aa_subst = alloc_printf("%s/%s", cwd, out_file);

      /* Construct a replacement argv value. */
//接下来将当前位置（*aa_loc = 0）的 '@' 换成空字节
      *aa_loc = 0;
//然后跳过aa_loc两个字节，拼接产生n_arg
...
// EXP_ST 主要被用于当afl被build成一个链接库时导出一些变量
//先定义一个struct用于存储信号相关信息
  struct sigaction sa;
  //设置
  sa.sa_handler   = NULL;
...
//然后使用sigaction()函数指定，当SIGHUP,SIGINT,SIGTERM
//信号来临时，信号处理函数是sa.sa_handler = handle_stop_sig
...
//SIGALRM：指定timeout的handler为handle_timeout函数
